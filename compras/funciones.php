...
function obtenerProductosEnCarrito()
{
    ...
    $sentencia = $bd->prepare("SELECT carrito.cant_art,articulos.itbis_art,articulos.id_art, articulos.nombre, articulos.des_art, articulos.precom_art
     FROM articulos
     INNER JOIN carrito_usuarios carrito
     ON articulos.id_art = carrito.id_producto
     WHERE carrito.id_sesion = ? AND concepto=2");
    $idSesion = session_id();
...
function quitarProductoDelCarrito($idArticulos)
{
    ...
    iniciarSesionSiNoEstaIniciada();
    $idSesion = session_id();
    $sentencia = $bd->prepare("DELETE FROM carrito_usuarios WHERE id_sesion = ? AND id_producto = ? AND concepto=2");
...
{
    $bd = obtenerConexion();
    $sentencia = $bd->query("SELECT id_art, nombre, des_art, precom_art FROM articulos WHERE status=1");
    return $sentencia->fetchAll();
}
...
function obtenerIdsDeProductosEnCarrito()
{
    $bd = obtenerConexion();
    iniciarSesionSiNoEstaIniciada();
    $sentencia = $bd->prepare("SELECT id_producto FROM carrito_usuarios WHERE id_sesion = ? AND concepto=2");
    $idSesion = session_id();
    $sentencia->execute([$idSesion]);
